...
class AssetHubPolkadot<TApi, TRes> extends Parachain<TApi, TRes> implements IPolkadotXCMTransfer {
  constructor(
    chain: TChainPolkadotKusama = 'AssetHubPolkadot',
  ...
  handleLocalReserveTransfer = <TApi, TRes>(
    ...
    const { api, version, destination, assetInfo: asset, paraIdTo } = input
    ...
    const call: TSerializedApiCall = {
      ...
      method: 'transfer_assets_using_type_and_then',
      parameters: {
        dest: createVersionedDestination(version, this.chain, destination, paraIdTo),
        assets: addXcmVersionHeader(
          [
            ...(useDOTAsFeeAsset ? [createAsset(version, PARA_TO_PARA_FEE_DOT, DOT_LOCATION)] : []),
        ...
        remote_fees_id: addXcmVersionHeader(
          useDOTAsFeeAsset ? DOT_LOCATION : asset.location,
          version
        ),
        fees_transfer_type: 'LocalReserve',
  ...
  patchInput<TApi, TRes>(
    ...
    const { assetInfo: asset, destination, version = this.version } = input
  ...
  async transferPolkadotXCM<TApi, TRes>(
    ...
    const CHAINS_SUPPORT_DOT_TRANSFER = new Set<TChainWithRelayChains>([
      'Hydration',
      'Polimec',
      'Moonbeam',
      'BifrostPolkadot',
