...
describe('getParaId', () => {
  ...
  it('should throw an error when the chain is invalid', () => {
    const mockChain = 'InvalidChain' as TChainDotKsmWithRelayChains

    vi.mocked(getChainConfig).mockImplementation(() => {
      throw new Error(`Chain configuration not found for ${mockChain}`)
    })

    expect(() => getParaId(mockChain)).toThrowError(
      `Chain configuration not found for ${mockChain}`
    )
    expect(getChainConfig).toHaveBeenCalledWith(mockChain)
  })

  it('should return the correct paraId for another valid chain', () => {
    const mockChain: TChainDotKsmWithRelayChains = 'Amplitude'
  ...
  it('should handle chains with a paraId of 0', () => {
    const mockChains: TChainDotKsmWithRelayChains = 'Polkadot'
