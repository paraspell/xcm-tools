...
describe('CurrenciesPallet.setBalance', () => {
  ...
  it('builds update_balance with numeric currency_id from string assetId', () => {
    ...
    const res = pallet.setBalance(address, asset)
  ...
  it('builds update_balance with numeric currency_id from numeric assetId', () => {
    const pallet = new CurrenciesPallet()
    const address = '5FbrsAddr'
  ...
  it('throws when assertHasId fails', () => {
    ...
    expect(() => pallet.setBalance(address, asset)).toThrow('missing id')
  })
})
