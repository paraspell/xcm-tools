...
import { reverseTransformLocation } from './reverseTransformLocation'

const clone = <T>(v: T): T => structuredClone(v)

describe('reverseTransformLocation', () => {
  it('adds {Parachain:1000} to an empty interior ("Here")', () => {
    const input: TLocation = { parents: Parents.ZERO, interior: 'Here' }
    const result = reverseTransformLocation(clone(input))
