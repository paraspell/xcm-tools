...
describe('handleExecuteTransfer', () => {
  ...
  it('should successfully create and return executeXcm transaction with hop', async () => {
    ...
    const result = await handleExecuteTransfer(input)
    ...
    expect(createExecuteCall).toHaveBeenNthCalledWith(1, input.chain, mockXcm, MAX_WEIGHT)
    expect(createExecuteCall).toHaveBeenNthCalledWith(2, input.chain, mockXcm, {
      proofSize: 0n,
      refTime: 12000n
    })
  })
