...
describe('submitTransaction', () => {
  ...
  it('also resolves on "txBestBlocksState" when found === true and ok === true', async () => {
    const { txMock, emit } = buildTxMock();
    ...
    const evt = {
      ...
      found: true,
      ok: true,
    } as unknown as TxFinalizedPayload;
    emit.next(evt);

    await expect(promise).resolves.toBe(evt);
  });

  it('rejects when ok === false in a terminal event', async () => {
    const { txMock, emit } = buildTxMock();
