import type { TAssetInfo, TChainPolkadotKusama, TPapiApi, TPapiTransaction } from '@paraspell/sdk';
...
describe('canBuildToExchangeTx', () => {
  ...
  const defaultOptions = {
    from: 'Polkadot' as TChainPolkadotKusama,
    amount: '10000000000',
  } as TGetBestAmountOutOptions;
  const exchangeChain: TChainPolkadotKusama = 'AssetHubPolkadot';
  ...
  it('should return { success: true } if "from" is undefined, without calling buildToExchangeExtrinsic', async () => {
    const options = { ...defaultOptions, from: undefined as unknown as TChainPolkadotKusama };
    const result = await canBuildToExchangeTx(
      options,
      exchangeChain,
      mockOriginApi,
  ...
  it('should handle TGetBestAmountOutOptions type for options input', async () => {
    ...
    const getBestAmountOutOptions = {
      from: 'Moonbeam' as TChainPolkadotKusama,
      amount: '5000000000000000000',
      currencyFrom: { symbol: 'GLMR' },
      currencyTo: { symbol: 'DOT' },
      to: 'Polkadot' as TChainPolkadotKusama,
    } as TGetBestAmountOutOptions;
