...
> {
  ...
  txFromHex(hex: string): Promise<TDedotExtrinsic> {
    const extrinsic = this.api.registry.$Extrinsic.tryDecode(hex);
    return Promise.resolve(this.api.toTx(extrinsic));
  }
  ...
  objectToHex(_obj: unknown, _typeName: string) {
    // TODO: Fix when DEDOT updates their API
  ...
  getTypeThenAssetCount(tx: TDedotExtrinsic): number | undefined {
    ...
    if (assets) {
      const firstValue = Object.values(assets)[0];
      return Array.isArray(firstValue) ? firstValue.length : undefined;
    }
    return undefined;
  }
