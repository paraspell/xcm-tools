...
import type { TChainDotKsmWithRelayChains } from '../src'
...
export const fetchTryMultipleProviders = <T>(
  ...
  getChainProviders: (chain: TChainDotKsmWithRelayChains) => string[],
...
export const fetchTryMultipleProvidersWithTimeout = async <T>(
  chain: TChainDotKsmWithRelayChains,
  fetchChainProviders: (chain: TChainDotKsmWithRelayChains) => string[],
  fetcher: (api: ApiPromise) => T
) => {
