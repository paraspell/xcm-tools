...
import { calculateFee } from '@paraspell/sdk-core/src/utils/ethereum/calculateFee'
...
import { ETH_CHAIN_ID, ETHEREUM_JUNCTION } from '@paraspell/sdk-core/src/constants'
...
export const executeTransferToEthereum = async ({
  ...
  ahAddress,
  currency
}: TEvmBuilderOptions<TPjsApi, Extrinsic>) => {
  ...
  const xcmHash = assetHubApi.getApi().createType('Xcm', customXcm)

  const sender = isEthersSigner(signer) ? await signer.getAddress() : signer.account

  const [parachainId, accountNextId] = await Promise.all([
    api.getApi().query.parachainInfo.parachainId(),
    api.getApi().rpc.system.accountNextIndex(sender as string)
  ])
  ...
  const xcmOnDest = assetHubApi.getApi().createType('XcmVersionedXcm', {
    V4: customXcm
  })
  const customXcmOnDest: BytesLike = xcmOnDest.toHex()

  const transferFee = await calculateFee(api)
