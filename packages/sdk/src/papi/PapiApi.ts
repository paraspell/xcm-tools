...
class PapiApi implements IPolkadotApi<PolkadotClient, TPapiTransaction> {
  ...
  async createApiInstance(wsUrl: string): Promise<PolkadotClient> {
    ...
    // let getWsProvider
    // if (isNodeJs) {
    //   getWsProvider = await import('polkadot-api/ws-provider/node').then(mod => mod.getWsProvider)
    // } else {
    //   getWsProvider = await import('polkadot-api/ws-provider/web').then(mod => mod.getWsProvider)
    // }
  ...
  callTxMethod({ module, section, parameters }: TSerializedApiCallV2): TPapiTransaction {
    ...
    console.log('transformedParameters', JSON.stringify(transformedParameters))
