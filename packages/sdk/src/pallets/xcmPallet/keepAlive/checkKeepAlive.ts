...
export const checkKeepAlive = async <TApi, TRes>({
  ...
  console.log('AMOUNT WITHOUT FEE: ', amountBN - xcmFee * BigInt(1.5))
  console.log('BALANCE + AMOUNT WITHOUT FEE: ', balance + (amountBN - xcmFee * BigInt(1.5)))
  console.log(
    'ORIGIN BALANCE - AMOUNT WITH FEE: ',
    balanceOrigin - (amountBN - xcmFee * BigInt(1.5))
  )

  const amountBNWithoutFee = amountBN - xcmFee * BigInt(1.5)
  ...
  const amountOriginBNWithoutFee = amountBN - xcmFee * BigInt(1.5)
