 ...
 */
export const getDefaultPallet = (chain: TChainDotKsmWithRelayChains): TPallet =>
  palletsMap[chain].defaultPallet
 ...
 */
export const getSupportedPallets = (chain: TChainDotKsmWithRelayChains): TPallet[] =>
  palletsMap[chain].supportedPallets.map(pallet => pallet.name)

export const getSupportedPalletsDetails = (chain: TChainDotKsmWithRelayChains): TPalletDetails[] =>
  palletsMap[chain].supportedPallets

export const getPalletIndex = (
  chain: TChainDotKsmWithRelayChains,
  pallet: TPallet
): number | undefined => palletsMap[chain].supportedPallets.find(p => p.name === pallet)?.index
