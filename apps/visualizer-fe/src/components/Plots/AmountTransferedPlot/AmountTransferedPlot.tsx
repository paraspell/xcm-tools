...
export const AmountTransferredPlot = forwardRef<HTMLDivElement, Props>(
  ({ counts, showMedian }, ref) => {
    ...
    const processData = () => {
      ...
      >((acc, item) => {
        ...
        acc[item.date][parachainKey] =
          Number(acc[item.date][parachainKey] || 0) + item.messageCount;
